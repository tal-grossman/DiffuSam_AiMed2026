...
\begin{minipage}[c][8.9cm][c]{0.70\textwidth}
  ...
  $^{2}$Dept.\ of Biomedical Engineering, Tel Aviv University, Israel}

  \vspace{4pt}

  {\color{white}\small
  \texttt{talgrossman22@gmail.com}}
  \end{center}
\end{minipage}\hfill
\begin{minipage}[c][8.9cm][c]{0.14\textwidth}
  \centering
...
% ── Keywords bar ───────────────────────────────────────────────
\colorbox{LightGray}{\parbox{\dimexpr\textwidth-2\fboxsep}{%
\vspace{4pt}
\textbf{Keywords:}\enspace
Medical image segmentation,\enspace diffusion models,\enspace SAM2,\enspace prompt-free segmentation,\enspace domain adaptation
\vspace{4pt}
}}
...
\vspace{4pt}
\textbf{We propose DiffuSAM}, a diffusion-based framework for \textbf{prompt-free} medical image segmentation that keeps the SAM2 backbone \textbf{entirely frozen}.
...
\vspace{0.08cm}
\begin{center}
\includegraphics[width=0.88\columnwidth]{pipeline_a_a.png}
\end{center}
\vspace{2pt}
{\small\textit{(a) DiffuSAM overview.} A diffusion prior (green) generates memory embeddings from SAM2 image encoder features, eliminating user prompts (red). All SAM2 components remain frozen.}

\vspace{0.08cm}
\begin{center}
\includegraphics[width=0.64\columnwidth]{pipeline_b.png}
\end{center}
\vspace{2pt}
{\small\textit{(b) Diffusion prior model.} During training: takes noised memory embedding, image embedding, timestep, and class label, learns to denoise. At inference: generates memory from pure noise in 2 steps.}
...
\begin{center}
\includegraphics[width=0.88\columnwidth]{inference_steps_patient_36_slice_32.png}\\[4pt]
...
\vspace{0.08cm}
\begin{center}
\IfFileExists{poster_qr.png}{%
  \includegraphics[width=0.30\columnwidth]{poster_qr.png}
}{%
  \begin{tikzpicture}
    \node[draw=SectionBlue, dashed, line width=2.5pt,
          rounded corners=6pt, minimum width=5.2cm, minimum height=5.2cm,
          text=SectionBlue, font=\small\bfseries, align=center]
